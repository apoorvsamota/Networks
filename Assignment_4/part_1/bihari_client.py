...
# Note: The import below is likely an error from copy-pasting and is not needed
# from Assignment_3.ryu.ryu.tests.unit import packet
...
class SelectiveRepeatClient:
    ...
    def process_in_order_packets(self):
        ...
        while self.expected_seq in self.buffer:
            ...
            # This logic is based on fixed-size chunks from server
            self.received_data.extend(data)
            del self.buffer[self.expected_seq]
            self.expected_seq += MAX_DATA_SIZE # Assuming fixed size
        ...
        while True:
            for packet in packets_to_process:
                ...
                if data == EOF_MARKER:
                    ...
                    # Fix for throughput calculation on zero time
                    if elapsed > 0:
                        print(f"[CLIENT] Throughput: {(len(self.received_data) * 8 / elapsed / 1_000_000):.2f} Mbps")
                    else:
                        print(f"[CLIENT] Throughput: N/A (transfer too fast)")
                ...
                if seq_num == self.expected_seq:
                    ...
                    self.received_data.extend(data)
                    self.expected_seq += MAX_DATA_SIZE # Assuming fixed size
                    # Check buffer for next in-order packets
