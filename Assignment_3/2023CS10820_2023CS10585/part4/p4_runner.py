...
def link_flap_exp(net, e, h1_ip, h2_ip, iperf_time = 30, link_down_duration = 5, wait_before_link_down = 2):
    ...
    Also captures OSPF control packets on s1 during the experiment.
    """
    h1, h2 = net.get("h1"), net.get("h2")
    s1 = net.get("s1") # Get a handle to the router s1
    ospf_log_file = "s1_ospf_packets.pcap"

    s_log, c_log = start_iperf(h1, h2, h1_ip, h2_ip, iperf_time)

    print(f"*** iperf running: client log {c_log}, server log {s_log}")

    # Start capturing OSPF (IP protocol 89) packets on router s1 to observe convergence.
    print(f"*** Starting OSPF packet capture on s1 -> {ospf_log_file}")
    s1.cmd(f"tcpdump -i any -w {ospf_log_file} ip proto 89 &")
    ...
    print("*** Stopping OSPF packet capture on s1.")
    s1.cmd('killall tcpdump')
    print(f"*** OSPF log saved to {ospf_log_file}. Use 'wireshark {ospf_log_file}' or 'tcpdump -r {ospf_log_file} -v' to inspect.")
