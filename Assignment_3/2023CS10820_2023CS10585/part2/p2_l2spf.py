# p2_l2spf.py - Per-Flow Symmetric Routing (Corrected)
...
class ShortestPathController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.logger.info("--- L2 Per-Flow Symmetric Controller Initialized ---")
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst_mac in self.host_location:
            ...
            chosen_path = random.choice(paths) if self.config.get('ecmp', False) and len(paths) > 1 else paths[0]
            self.logger.info("New flow %s -> %s (ports %s->%s). Chosen path: %s", 
                             src_ip, dst_ip, src_port, dst_port, chosen_path)

            self._install_symmetric_flow(chosen_path, src_mac, dst_mac, src_ip, dst_ip, ip_proto, src_port, dst_port)
