#!/usr/bin/env python3
# p4_sdn_runner.py - SDN experiment runner with link failure and comparison graphs
...
def start_ryu_controller():
    """Start Ryu controller as subprocess"""
    info('*** Starting Ryu controller\n')
    ryu_proc = subprocess.Popen(
        ['ryu-manager', '--observe-links', 'p4_l3spf_lf.py'],
...
def stop_ryu_controller(ryu_proc):
    """Stop Ryu controller"""
    if ryu_proc:
...
def plot_comparison(sdn_data, ospf_data, output_file='throughput_comparison.png'):
    ...
    ax.axvline(x=2, color='gray', linestyle='--', alpha=0.5, label='Link Down')
    ax.axvline(x=7, color='green', linestyle='--', alpha=0.5, label='Link Up')
    ax.text(2, ax.get_ylim()[1] * 0.95, 'Link Down', rotation=90, 
            verticalalignment='top', fontsize=9, color='gray')
    ax.text(7, ax.get_ylim()[1] * 0.95, 'Link Up', rotation=90,
...
def calculate_convergence_time(throughput_data, failure_time=2, recovery_time=7, threshold=50):
    """
    Calculate convergence time after link failure and recovery
    Convergence is when throughput returns above threshold
    """
    down_convergence = None
    up_convergence = None
    
    for time, throughput in throughput_data:
        # After link goes down at failure_time
        if down_convergence is None and time > failure_time and throughput > threshold:
            down_convergence = time - failure_time
        
        # After link comes up at recovery_time
        if up_convergence is None and time > recovery_time and throughput > threshold:
...
def main():
    ...
    try:
        ...
        print("*** Waiting for SDN controller to discover topology...")
        time.sleep(5)
        ...
        if os.path.exists(args.ospf_log):
            ...
            print(f"*** OSPF log not found at {args.ospf_log}, skipping comparison")
        
        # Calculate convergence times
        if sdn_data:
            sdn_down_conv, sdn_up_conv = calculate_convergence_time(
                sdn_data, 
                failure_time=args.wait_before_down,
                recovery_time=args.wait_before_down + args.link_down_duration
            )
            print(f"\n*** SDN Convergence Times:")
            print(f"    After link down: {sdn_down_conv:.2f}s" if sdn_down_conv else "    After link down: N/A")
            print(f"    After link up: {sdn_up_conv:.2f}s" if sdn_up_conv else "    After link up: N/A")
        
        if ospf_data:
            ospf_down_conv, ospf_up_conv = calculate_convergence_time(
                ospf_data,
                failure_time=args.wait_before_down,
                recovery_time=args.wait_before_down + args.link_down_duration
            )
            print(f"\n*** OSPF Convergence Times:")
            print(f"    After link down: {ospf_down_conv:.2f}s" if ospf_down_conv else "    After link down: N/A")
            print(f"    After link up: {ospf_up_conv:.2f}s" if ospf_up_conv else "    After link up: N/A")
        
        # Generate comparison plot
        if sdn_data or ospf_data:
            print("\n*** Generating throughput comparison graph...")
            plot_comparison(sdn_data, ospf_data)
