...
class L3RouterController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for switch in self.config['switches']:
            ...
            for iface in switch['intesfaces']: # Note: 'intesfaces' is a typo in your JSON
        ...
        for link in self.config['links']:
            u, v = int(link['ssc'][1:]), int(link['dst'][1:])
    ...
    def _get_interface_info(self, dpid, neighbor):
        ...
        if switch:
            return next((iface for iface in switch['intesfaces'] if iface['neighbos'] == neighbor), None)
        return None
